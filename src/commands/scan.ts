...
export const scanCommand = new Command('scan')
  ...
  .option('--no-wait', 'Start scan without waiting for results')
  ...
  .action(async (domain: string, options: any) => {
    try {
      ...
      if (!options.wait) {
        if (options.json) {
          console.log(JSON.stringify({ scan_id: scanId, status: 'started' }));
        } else {
          console.log(`Scan started (ID: ${scanId})`);
          console.log(`Check status: rampart scan status ${scanId}`);
          console.log(`Wait for results: rampart scan ${domain}`);
        }
        return;
      }
